...
for field, data_ in grouped_df:
    ...
    if cmd_args.varianceApproach == "Maguio":
        variance_est = sigma_fit_sigma_clip**2 + m**2 * data_["ierr"]**2 + (1 - m)**2 * data_["rerr"]**2 + data_["F660err"]**2
...
df_file = "Ha-emitters-disk/Halpha-{}-{}_PerField.csv".format(file_.split('.cs')[0], cmd_args.varianceApproach) 
ha_emitter_combined.to_csv(df_file, index=False)

asciifile = "Ha-emitters-disk/Halpha-{}-{}_PerField.ecsv".format(file_.split('.cs')[0], cmd_args.varianceApproach) 
