...
# The Halpha emitters
df_halpha_d = pd.read_csv("Ha-emitters/Halpha_Mine_PerField_total-unique.csv")

# Define the number of bins and the range of r-band magnitudes
num_bins = 50
mag_range_d = (min(df_halpha_d["r_PStotal"].min(), df_all_objects_d["r_PStotal"].min()), max(df_halpha_d["r_PStotal"].max(), df_all_objects_d["r_PStotal"].max()))
...
number_density_halpha_d = np.histogram(df_halpha_d["r_PStotal"], bins=num_bins, range=mag_range_d)[0] / bin_width_d / 347.4
number_density_all_objects_d = np.histogram(df_all_objects_d["r_PStotal"], bins=num_bins, range=mag_range_d)[0] / bin_width_d / 347.4
...
plt.hist(np.arange(len(number_density_halpha_d)), bins=bin_edges, weights=number_density_halpha_d, color='deepskyblue', label='Hα Excess', alpha=1, zorder =3,
         histtype='step', linewidth=3, density=True)
plt.hist(np.arange(len(number_density_all_objects_d)), bins=bin_edges, weights=number_density_all_objects_d, color='salmon', label='All Stars', alpha=1,  zorder =2,
         histtype='step', linewidth=3, density=True)
...
plt.tick_params(axis='x', labelsize=20)
plt.tick_params(axis='y', labelsize=20)
plt.hist(df_halpha_d["r_PStotal"], bins=num_bins, range=mag_range_d, alpha=1, color='deepskyblue', label='Hα Excess', cumulative=True, density=True,  zorder =3)
plt.hist(df_all_objects_d["r_PStotal"], bins=num_bins, range=mag_range_d, alpha=1, color='salmon', label='All Stars', cumulative=True, density=True,  zorder =2)
...
plt.tick_params(axis='x', labelsize=20)
plt.tick_params(axis='y', labelsize=20)
sns.kdeplot(df_halpha_d["r_PStotal"], color='navy', label='Hα Excess', linestyle='-', linewidth=2,  zorder =3)
sns.kdeplot(df_all_objects_d["r_PStotal"], color='salmon', label='All Stars', linestyle='--', linewidth=2,  zorder =2)
plt.xlabel("r-band Magnitude", fontsize=20)
...
df_d_h = df_halpha_d.rename(columns={"RA": "RA", "DEC": "DEC"})
...
plt.tick_params(axis='x', labelsize=20)
plt.tick_params(axis='y', labelsize=20)
plt.hist(df_halpha_d["GAL_LONG"], bins=num_bins, range=lon_range, alpha=1, color='deepskyblue', label='Hα Excess', cumulative=True, density=True, zorder=3)
plt.hist(galactic_coords_d.l.deg, bins=num_bins, range=lon_range, alpha=1, color='salmon', label='All Stars', cumulative=True, density=True, zorder=2)
...
plt.tick_params(axis='x', labelsize=20)
plt.tick_params(axis='y', labelsize=20)
sns.kdeplot(df_halpha_d["GAL_LONG"], color='navy', label='Hα Excess', linestyle='-', linewidth=2, zorder=3)
sns.kdeplot(galactic_coords_d.l.deg, color='salmon', label='All Stars', linestyle='--', linewidth=2, zorder=2)
