...
with sns.axes_style('white'):
    ...
    scatter = ax1.scatter(
        data_final['r - i'], data_final['r - J0660'],
        s=5, 
        color="gray",
    ...
    ax1.plot(x_values, fitted_line_normal(x_values), 'k-', zorder=6, label='Initial fitted')
    ax1.plot(x_values, fitted_line_sigma_clip(x_values), ls='--', color="k", zorder=8, label='Iter. fitted $\sigma$ clipped')
    ax1.plot(x_values, fitted_line_sigma_clip(x_values) + 4 * sigma_fit_sigma_clip, ls=':', color="k", zorder=8, label='Representation 4$\sigma$')
    plt.fill_between(x_values, fitted_line_sigma_clip(x_values), fitted_line_sigma_clip(x_values) + 4 * sigma_fit_sigma_clip, color='k', alpha=0.1)
    ...
    file_save = "diagram-{}-maguio.jpg".format(file_.split('.cs')[0])
...
df_file = "Halpha-{}-maguio.csv".format(file_.split('.cs')[0]) 
data_ha_emitter.to_csv(df_file, index=False)

asciifile = "Halpha-{}-maguio.ecsv".format(file_.split('.cs')[0]) 
Table.from_pandas(data_ha_emitter).write(asciifile, format="ascii.ecsv", overwrite=True)
