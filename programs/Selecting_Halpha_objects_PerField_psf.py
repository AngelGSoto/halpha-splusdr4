...
for field, data_ in grouped_df:
    cx, cy = colour(data_, "r_psf", "i_psf", "r_psf", "J0660_psf")
    ...
    data = pd.concat([data_["RA"], data_["DEC"], df["FWHM"], data_["r_psf"], colum1, colum2], axis=1)
    ...
    ecx, ecy = errormag(data_, "e_r_psf", "e_i_psf", "e_r_psf", "e_J0660_psf")
    ...
    if cmd_args.varianceApproach == "Maguio":
        variance_est = sigma_fit_sigma_clip**2 + m**2 * data_["e_i_psf"]**2 + (1 - m)**2 * data_["e_r_psf"]**2 + data_["e_J0660_psf"]**2
...
df_file = "Ha-emitters-psf/Halpha-{}-{}_PerField_psf.csv".format(file_.split('.cs')[0], cmd_args.varianceApproach) 
ha_emitter_combined.to_csv(df_file, index=False)

asciifile = "Ha-emitters-psf/Halpha-{}-{}_PerField_psf.ecsv".format(file_.split('.cs')[0], cmd_args.varianceApproach) 
