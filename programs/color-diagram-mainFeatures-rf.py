...
fig, axes = plt.subplots(2, 3, figsize=(20, 12), dpi=300)  # 2 rows, 3 columns
...
for i, ((x1, y1), (x2, y2)) in enumerate(specific_pairs):
    ...
    for group, (label, group_data) in enumerate(df_splus_wise.groupby("Label")):
        ...
        ax.scatter(group_data[x1] - group_data[y1], group_data[x2] - group_data[y2],
                   c=[color], s=150, marker=marker, edgecolors=[edge_color], linewidth=0.5, label=legend_label, alpha=0.7)
...
plt.tight_layout()
plt.savefig("Figs/color_color_diagrams_multiple.pdf", format='pdf', bbox_inches='tight', dpi=300)
plt.close()
